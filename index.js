...
app.post("/addnamelistss", (req, res) => {
  ...
  (err,user0) => {
    if(err){
      ...
      db.query("Update user_info set Status_user = 0 where Email = ? ",
      [email],
      (err,user_up) => {
        if(err){
          console.log("err addnamelistss Update = ======> " , err)
        }
      })
      res.send({massage:err})
    } else {
    ...
    db.query(
      ...
      (err, user1) => {
        try {
          if (user1.length == 0) {
            db.query(
              "INSERT INTO participant (User_id,Id_event,Email,Password,Token,Authen,ElecStatus) VALUEs(?,0,?,?,?,'admin',0)",
    ...
    db.query(
      ...
      (err, user1) => {
        try {
          ...
          if (user1.length == 0) {
            db.query(
              "INSERT INTO participant (User_id,Id_event,Email,Password,Token,Authen,ElecStatus) VALUEs(?,?,?,?,?,'staff',0)",
...
app.get("/lengthcandidate", (req, res) => {
  try{
    ...
    console.log("/lengthcandidate ============= > ", err);
  } 
});
...
app.get("/img/:fileName", (req, res) => {
  try{
    ...
    const filepath = path.resolve(`./public/img/${fileName}.png`);
    console.log(filepath)
    res.sendFile(filepath);
  }catch(err){
    console.log("/img/:fileName =======> " ,err)
  } 
});
...
app.post("/upload", (req, res) => {
  ...
  base64Img.img(imagStr, "public/img", fileName, (err, filepath) => {
    if (err) {
      console.log(err);
      return res.status(500).send({ message: "Uploadimagefailed" });
    } else {
      console.log("/Upload")
      res.send({massage : "Upload"});
      try{
                      ...
                      db.query("select * from candidate_info", (err, user2) => {
                        ...
                        if (user2.length == 0) {
                          db.query(
                            ...
                            [Id_event, id_user, detailc, img], //classifiedsadd = image
                            (err, user) => {
                              if (err) {
...
//   destination: path.join("./images", "images"),
...
app.post("/addparticipant", (req, res) => {
  ...
  const base64 = buff.toString("base64");
  try{
    console.log("event_now ===>>>> " , event_now)
    db.query(
      "select * from event_info where Event_name = ?",
      [event_now],
      (err, user1) => {
                ...
                if (user2.length != 0) {
                  ...
                  db.query("SELECT * FROM participant", (err, user3) => {
                    ...
                    if (
                      ...
                      db.query(
                        "INSERT INTO participant (Par_id , User_id , Id_event , Email , Password , Token , Authen , ElecStatus) "+
                        " VALUEs(?,?,?,?,?,?,?,0)",
                              ...
                              (err, user4) => {
                                ...
                                if (user4.length == 0) {
                                  if (
                                    ...
                                    idp = user3[user3.length - 1].Par_id + 1;
                                    db.query(
                                      "INSERT INTO participant (Par_id,User_id,Id_event,Email,Password,Token,Authen,ElecStatus) VALUEs(?,?,?,?,?,?,?,0)",
                                      [
                                        idp,
                                        key,
                                        idE,
...
app.post("/checkno" ,(req,res) => {
  var eventnow = req.body.eventnow;
  try{
    console.log(eventnow);
    db.query(
      "SELECT  Id_can , event_info.Id_event FROM `event_info` "+
      ...
      (err, user) => {  
        ...
        if(user.length == 1) {
          ...
          db.query(
            "SELECT  Par_id , event_info.Id_event FROM `event_info`  "+
            " JOIN participant "+
      ...
      const mailOptions = {
        ...
        html:
          ...
          "และสิ้นสุดเวลา :"+event_end+
           "</b> "+
           "<b> <a href='http://46.137.201.121:5000'> เชิญเข้ารับการเลือกตั้ง </a> </b> </div>", // HTML body
      };
  ...
  const transporter = nodemailer.createTransport({
      ...
      const mailOptions = {
        ...
        html:
          ...
          "<br></br> <b> <a href='http://46.137.201.121:5000'> เชิญเข้ารับการเลือกตั้ง </a> </b> </div>", // HTML body
      };
      transporter.sendMail(mailOptions, function (err, info) {
        if (err) { 
          res.send(err);
...
app.post("/showadd_candidate", (req, res) => {
  ...
  db.query(
    ...
    console.log("/showadd_candidate ========> ", ree)
  }
...
app.get("/showeventmanage", (req, res) => {
  try{
            ...
            (err, user2) => {
              if (err) {
                ...
                db.query(
                  ...
                  " GROUP BY candidate_info.Id_event;",
                  (err, user3) => {
                    if (err) {
                      console.log("err show event manage = ", err);
                    } else {
    ...
    console.log("//showeventmanage =============> "  ,err )
  } 
});
...
app.post("/addparticipant_Edit", (req, res) => { 
  ...
  try{
    ...
    console.log("/addparticipant_Edit   =================================> " , err)
  } 
});
...
app.post("/Showdetailevent", (req, res) => {
  ...
  try{
    ...
    console.log("/Showdetailevent =============> " , err)
  } 
});
...
app.get("/showStaff", (req, res) => {
  try{
    db.query(
      " select participant.User_id, participant.Par_id as id, user_info.Name as Name, user_info.Surname as Surname , user_info.Email as Email, participant.Authen as Authen " +
        " from participant LEFT JOIN user_info " +
        " ON participant.User_id = user_info.User_id" +
        " WHERE participant.Authen = 'admin' or participant.Authen = 'staff'" +
        "   GROUP BY participant.Par_id  " +
        " ORDER BY participant.Par_id;",
      (err, user) => {
        if (err) {
          // console.log("err", err);
...
app.post("/showeventstat", (req, res) => {
  db.query(
    "select * from event_info where not Id_event = 0 And not status_event = 4 ",
    (err, user) => {
      if (err) {
        // console.log("err");
        res.send({ err, massage: "dont show" });
...
// Show Staff Event ----------------------------------------------------------------- ถึงตรงนี้แล้ว
...
app.post("/showeventStaff", (req, res) => {
  try{
    db.query(
      ...
      (err, user) => {
        if (err) {
          ...
          db.query(
            "SELECT COUNT(Par_id) as nopar  , Id_event FROM `participant` Where not Authen = 'staff'  and not Id_event = 0 GROUP BY Id_event ",
            (err, user2) => {
              if (err) {
                // console.log("err show event manage = ", err);
              } else {
                db.query(
                  "SELECT COUNT(Id_can) as nocan , Id_event FROM `candidate_info` Where not Id_event = 0 GROUP BY Id_event ",
                  (err, user3) => {
                    if (err) {
                      // console.log("err show event manage = ", err);
                    } else {
    ...
    console.log("/showeventStaff ======> " , err)
  } 
});
...
app.post("/showrevent_result", (req, res) => { 
  ...
  try{
    ...
    console.log("/showrevent_result   ========= > " , err)
  } 
});
...
app.post("/votestat", (req, res) => {
  ...
  try{
    db.query(
      "SELECT  COUNT(Par_id)  AS par FROM `participant`  WHERE Id_event = 1 AND NOT Authen = 'staff' AND ElecStatus = 0 GROUP BY ElecStatus;",
      [Id_event],
      (err, user) => {
        if (err) {
      ...
      if(err){
        ...
        db.query(
          "SELECT user_info.Name , user_info.Surname ,COUNT(Vote) AS COUNT , candidate_info.Detail , candidate_info.Img as Img" +
            ...
            " where candidate_info.Id_event = ? AND NOT candidate_info.Id_can = 0  GROUP BY Vote" +
            " ORDER by COUNT DESC",
          [id_event],
          (err, user) => {
            if (err) {
        ...
        if (err) {
          ...
          db.query(
            "select event_info.Id_event , participant.User_id , event_info.Event_name , event_info.Date_vote , event_info.Time_start , event_info.Time_end , event_info.status_event" +
              ...
              " LEFT JOIN event_info" +
              " ON participant.Id_event = event_info.Id_event" +
              " Where participant.User_id = ? And Authen = 'staff'",
            [idu[0].User_id],
            (err, idp) => {
              if (err) {
...
app.post("/Csv_data_check", (req, res) => {
  ...
  try{
    ...
    console.log("/Csv_data_check ======================> ", err)
   } 
});
  ...
  try{
    ...
    db.query(
      "SELECT event_info.Event_name ,event_info.Id_event as Id_event ,event_info.status_event FROM `participant` " +
        ...
        " AND (event_info.status_event = 0 or event_info.status_event = 4) ",
      [ID_staff],
      (err, user) => {
        if (err) {
          // console.log("err");
          ...
          db.query(
            ...
            " AND Id_event = " + user[0].Id_event +" GROUP BY Id_event ;",
            (err, user2) => {
              if (err) {
                // console.log("err show event manage = ", err);
              } else {
                db.query(
                  ...
                  " And not Id_can = 0 GROUP BY Id_event  ",
                  (err, user3) => {
                    if (err) {
                      // console.log("err show event manage = ", err);
                    } else { 
    ...
    console.log("/staff_showeventmanage    ==================> "  ,err)
  } 
});
...
app.post("/Staff_showeventstat", (req, res) => {
  ...
  try{
    ...
    console.log("/Staff_showeventstat  ==========================> " ,err)
  } 
});
